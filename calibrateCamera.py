...
for cam in range(1, 7):
    head_foot_image = np.loadtxt(f'matchings/Camera{cam}.txt')
    head_foot_image = np.concatenate([head_foot_image[:, 2:4], head_foot_image[:, 4:]], axis=0). \
        reshape([1, -1, 2]).astype('float32')
    head_foot_3d = np.loadtxt(f'matchings/Camera{cam}_3d.txt')
    ...
    f = cv2.FileStorage(f'intrinsic/intr_Camera{cam}.xml', flags=cv2.FILE_STORAGE_WRITE)
    f.write(name='camera_matrix', val=cameraMatrix)
    f.write(name='distortion_coefficients', val=distCoeffs)
    f.release()
    f = cv2.FileStorage(f'extrinsic/extr_Camera{cam}.xml', flags=cv2.FileStorage_WRITE_BASE64)
